...
;(function($, window, document, undefined) {
    ...
    let resizingHandler = function(force) {
        let execute = typeof(force) === 'undefined' ? false : (force === true);
        let new_width = $(window).outerWidth();
        if(execute || new_width != window.current_window_width){
            ...
            setTimeout(function(){
                $('.slider-overlay').each(function(){
                    let $so  = $(this);
                    let $fs  = $so.next('flexslider').children('.flex-viewport').first();
                    let so_h = $so.outerHeight();
                    let fs_h = $fs.outerHeight();
                    if(so_h > fs_h){
    ...
    $document.ready(function(){
        ...
        let alternate_menu_behavior = $.browser.safari || isMobile.iOS();
		...
		$('#options a').not('.sub-nav a, .sub-nav-toggle').on('click', function(e){
